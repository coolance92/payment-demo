...
public class RefundInfoServiceImpl extends ServiceImpl<RefundInfoMapper, RefundInfo> implements RefundInfoService {
    ...
    public void updateRefund(String content) {
        ...
        //将json字符串转换成Map
        Gson gson = new Gson();
        Map<String, Object> resultMap = gson.fromJson(content, new TypeToken<Map<String, Object>>() {
        }.getType());
        ...
        refundInfo.setRefundId((String) resultMap.get("refund_id"));//微信支付退款单号

        //查询退款和申请退款中的返回参数
        if (resultMap.get("status") != null) {
            refundInfo.setRefundStatus((String) resultMap.get("status"));//退款状态
            refundInfo.setContentReturn(content);//将全部响应结果存入数据库的content字段
        }
        //退款回调中的回调参数
        if (resultMap.get("refund_status") != null) {
            refundInfo.setRefundStatus((String) resultMap.get("refund_status"));//退款状态
